...
async def _get_diagnostic_questions(
    ...
    """Build a short balanced diagnostic without consuming Free attempts."""
    use_questions = await DBClient.get_personalized_tasks(
        user_id=user_id,
        category=CATEGORY_USE_OF_ENGLISH,
        limit=8,
    )
    reading_questions = await DBClient.get_personalized_tasks(
        user_id=user_id,
        category=CATEGORY_READING,
        limit=4,
    )

    questions = use_questions + reading_questions

    if len(questions) < 12:
        return []

    # Keep the diagnostic deterministic enough to be balanced, but still varied.
    import random
    random.shuffle(questions)
    return questions[:min(12, len(questions))]
