...
class LearningDB:
    ...
    @staticmethod
    async def get_or_create_session(user_id: int, session_type: str, length: int, practice_mode: str = "full") -> Dict[str, Any]:
        target_id = int(user_id)
        ...
        safe_max = 12 if session_type == "diagnostic" else (10 if session_type == "practice" else 5)
        safe_length = max(3, min(int(length), safe_max))

        def _op():
            if session_type == "practice":
                mode = practice_mode if practice_mode in {"focus", "challenge", "full"} else "full"
                result = supabase.rpc("create_practice_session_v7", {
                    "p_user_id": target_id,
                ...
                if not isinstance(data, dict) or not data.get("id"):
                    raise RuntimeError("create_practice_session_v7 returned no session")
                return data

            # Never create a second active session from an old Telegram button.
            active = (
            ...
            try:
                ...
                if active_after_race.data:
                    return active_after_race.data[0]
                raise
            return payload
    ...
    @staticmethod
    async def get_session_xp(session_id: str, user_id: int) -> int:
        def _op():
            result = supabase.rpc("get_session_xp_v6", {
                "p_session_id": str(session_id),
                "p_user_id": int(user_id),
            }).execute()
            data = result.data
    ...
    @staticmethod
    async def get_today_daily_session(user_id: int) -> Optional[Dict[str, Any]]:
        today = datetime.now(KYIV).date().isoformat()
        def _op():
            result = (
                supabase.table("learning_sessions")
    ...
    async def get_progress(user_id: int) -> Dict[str, Any]:
        ...
        def _op():
            result = supabase.rpc("get_user_learning_progress_v6", {"p_user_id": target_id}).execute()
            data = result.data or {}
            if isinstance(data, list):
                data = data[0] if data else {}
            ...
            return {
                ...
                "xp_total": int(data.get("xp_total") or 0),
                "xp_week": int(data.get("xp_week") or 0),
                "skills": skills[:20],
            }
    ...
    async def set_reminder(user_id: int, enabled: bool, reminder_time: Optional[str] = None) -> None:
        target_id = int(user_id)
        payload: Dict[str, Any] = {"reminder_enabled": bool(enabled), "reminder_timezone": "Europe/Kyiv", "reminder_preference_set": True}
        if enabled:
            payload["reminder_resume_at"] = datetime.now(timezone.utc).isoformat()
        if reminder_time:
    ...
    async def leaderboard(user_id: int) -> Dict[str, Any]:
        ...
        def _op():
            result = supabase.rpc("get_weekly_xp_leaderboard_v6", {
                "p_user_id": int(user_id),
    ...
    async def finish_reminder(delivery_id: str, status: str) -> None:
        ...
        await DBClient._run_sync(lambda: supabase.table("learning_reminder_deliveries").update(payload).eq("id", delivery_id).eq("status", "claimed").execute())

    @staticmethod
    async def click_reminder(delivery_id: str, user_id: int) -> None:
        await DBClient._run_sync(lambda: supabase.table("learning_reminder_deliveries").update({"clicked_at": datetime.now(timezone.utc).isoformat()}).eq("id", delivery_id).eq("user_id", int(user_id)).is_("clicked_at", "null").execute())
    ...
    @staticmethod
    async def get_motivation(user_id: int) -> Dict[str, Any]:
        def _op():
            result = supabase.table("users").select("level,xp,streak,leaderboard_opt_in").eq("id", int(user_id)).limit(1).execute()
            return result.data[0] if result.data else {}
        try:
            return await DBClient._run_sync(_op)
        except Exception:
            logger.exception("Could not load optional motivation data")
            return {}
