    ...
    async def get_progress(user_id: int) -> Dict[str, Any]:
        ...
        def _op():
            user_rows = (
                ...
                .select("total_questions_answered,total_correct_answers,total_tasks_solved,streak,last_streak_date")
                .eq("id", target_id)
                .limit(1)
                .execute()
            ).data or []
            ...
            topic_rows = (
                ...
                .select("category,sub_category,attempts,correct,mastery_score")
                .eq("user_id", target_id)
                .execute()
            ).data or []
            ...
            for row in topic_rows:
                ...
                skills.append({
                    ...
                    "mastery": round(float(row.get("mastery_score") or 0)),
                })

            # Fallback for installations where legacy user_topic_progress was not populated.
            if not skills:
                answers = (
                    supabase.table("learning_session_answers")
                ...
                qids = list({str(a.get("question_id")) for a in answers if a.get("question_id")})
                questions: Dict[str, Dict[str, Any]] = {}
                if qids:
                    qrows = supabase.table("questions").select("id,category,topic,sub_category").in_("id", qids).execute().data or []
                    questions = {str(q["id"]): q for q in qrows}
                stats: Dict[str, Dict[str, int]] = {}
                for a in answers:
                    ...
                    item = stats.setdefault(name, {"correct": 0, "attempts": 0})
                    item["attempts"] += 1
                    item["correct"] += 1 if a.get("is_correct") else 0
                for name, item in stats.items():
                    attempts = item["attempts"]
                    skills.append({
                        "skill": name,
            ...
            total_attempts = int(user.get("total_questions_answered") or user.get("total_tasks_solved") or 0)
            total_correct = int(user.get("total_correct_answers") or 0)

            # Preserve old learning-day history from legacy answers while using the
            # new learning_days table for the current flow.
            day_rows = supabase.table("learning_days").select("learning_date").eq("user_id", target_id).limit(1000).execute().data or []
            legacy_rows = (
            ...
            learning_dates = {str(x.get("learning_date")) for x in day_rows if x.get("learning_date")}
            for item in legacy_rows:
                raw = item.get("answered_at")
                if not raw:
                    continue
                try:
                    dt = datetime.fromisoformat(str(raw).replace("Z", "+00:00")).astimezone(KYIV)
                    learning_dates.add(dt.date().isoformat())
                except ValueError:
                    continue

            if total_attempts <= 0 and legacy_rows:
                # Older deployments may not have maintained the counters reliably.
                answer_rows = (
                    supabase.table("user_answers")
                    .select("is_correct")
                    .eq("user_id", target_id)
                ...
                total_attempts = len(answer_rows)
                total_correct = sum(1 for x in answer_rows if x.get("is_correct"))
