...
# Server-side secret/service-role key is preferred. SUPABASE_KEY remains a
# backward-compatible fallback for existing Render deployments.
SUPABASE_KEY: str = (
    os.getenv("SUPABASE_SERVICE_ROLE_KEY", "").strip()
    or _get_required_env("SUPABASE_KEY")
)
WEBHOOK_BASE_URL: str = _get_required_env("WEBHOOK_BASE_URL")

# Telegram can sign every webhook request with a secret header. To avoid one
# more mandatory deployment secret, derive a stable token from BOT_TOKEN unless
# WEBHOOK_SECRET is explicitly supplied. The bot token itself is never exposed.
WEBHOOK_SECRET: str = os.getenv("WEBHOOK_SECRET", "").strip() or hashlib.sha256(
    f"neta-webhook:{BOT_TOKEN}".encode("utf-8")
).hexdigest()
...
REFERRAL_INVITEE_PREMIUM_DAYS: int = 1

# Manual payment / sales
MANAGER_USERNAME: str = os.getenv("MANAGER_USERNAME", "").strip().lstrip("@")
...
# Emergency switch: false disables all scheduled reminders without changing preferences.
RETENTION_ENABLED: bool = os.getenv("RETENTION_ENABLED", "true").lower() in {"true", "1", "yes"}
