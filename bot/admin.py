...
async def admin_stats(
    ...
    text = (
        ...
        f"📚 Питань: "
        f"<b>{stats['total_questions']}</b> · активні й схвалені: {stats.get('active_questions', 0)}\n"
...
async def admin_growth_funnel(
    ...
    try:
        data = await LearningDB.product_funnel(days=30)
    except Exception:
    ...
    def stage(label, key):
        ...
        return f"{label}: <b>{value}</b> · {_percent(value, starts)} від нових /start"
    lines = [
        "📈 <b>Продукт · 30 днів</b>",
        "Нові = перший записаний /start за весь час. Тестові акаунти виключені.",
        f"🚪 Нові /start: <b>{starts}</b>",
        stage("Перше заняття почали", "intro_start"),
        stage("Перше заняття завершили", "intro_done"),
        stage("Daily почали", "daily_start"),
        stage("Daily завершили", "daily_done"),
        stage("Додаткове завершили", "practice_done"),
        stage("Курс переглянули", "course_view"),
        stage("Залишили інтерес до курсу", "course_interest"),
        "\n<b>Daily retention</b> · від першого завершеного Daily; лише повні дні за Києвом",
    ]
    for key in ("d1", "d3", "d7"):
        row = (data.get("retention") or {}).get(key) or {}
        n, d = int(row.get("retained") or 0), int(row.get("eligible") or 0)
        lines.append(f"{key.upper()}: <b>{n}/{d}</b> · {_percent(n,d)}")
    sent = int(data.get("reminders_sent") or 0)
    clicked = int(data.get("reminders_clicked") or 0)
    returned = int(data.get("reminders_returned") or 0)
    lines += [
        "\n<b>Автоматичні підказки · усі користувачі</b>",
        f"Надіслано: <b>{sent}</b>",
        f"Натиснули: <b>{clicked}</b> · {_percent(clicked,sent)}",
        f"Завершили заняття протягом 24 год: <b>{returned}</b> · {_percent(returned,sent)}",
