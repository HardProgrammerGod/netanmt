...
async def admin_stats(
    ...
    text = (
        ...
        f"✅ Не заблокували бота: "
        f"<b>{stats['active_users']}</b>\n"
        f"🚫 Заблокували бота: "
        f"<b>{stats['blocked_users']}</b>\n"
        f"🧠 Розв'язали ≥1 завдання: "
        f"<b>{stats.get('learners_started', 0)}</b>\n"
        f"📅 Були в боті за 7 днів: "
        f"<b>{stats.get('active_7d', 0)}</b>\n"
        f"⚡ Виконали хоча б один Daily: "
        f"<b>{stats.get('daily_users', 0)}</b>\n"
        f"⭐ Premium зараз: "
        f"<b>{stats['premium_users']}</b>\n"
        f"💳 Платили Stars: <b>{stats.get('paying_users', 0)}</b> · "
        f"виручка <b>{stats.get('stars_revenue', 0)} ⭐</b>\n"
        f"📚 Питань: "
        f"<b>{stats['total_questions']}</b>\n"
        f"🔗 Переходів за реф. посиланням: "
        f"<b>{stats.get('referral_starts', 0)}</b>\n"
        f"🎁 Активованих рефералів: "
...
@admin_router.callback_query(F.data == "admin_growth_funnel")
async def admin_growth_funnel(
    ...
    await callback.answer("Збираю воронку...")

    try:
        data = await DBClient.get_growth_funnel(days=30)
    except Exception:
        logger.exception("Не вдалося побудувати growth funnel.")
        await callback.message.edit_text(
            "⚠️ Не вдалося побудувати воронку. Спробуй ще раз пізніше.",
            reply_markup=get_admin_keyboard(),
        )
        return

    counts = data.get("counts") or {}
    starts = int(counts.get("bot_started") or 0)
    diag_started = int(counts.get("diagnostic_started") or 0)
    diag_completed = int(counts.get("diagnostic_completed") or 0)
    daily_started = int(counts.get("daily_started") or 0)
    daily_completed = int(counts.get("daily_completed") or 0)
    training_started = int(counts.get("training_started") or 0)
    training_completed = int(counts.get("training_completed") or 0)
    premium_opened = int(counts.get("premium_opened") or 0)
    payment_started = int(counts.get("stars_payment_started") or 0)
    payment_completed = int(counts.get("stars_payment_completed") or 0)
    manager_clicked = int(counts.get("manager_payment_clicked") or 0)
    referral_joined = int(counts.get("referral_joined") or 0)
    referral_activated = int(counts.get("referral_activated") or 0)
    referral_rewards = int(counts.get("referral_reward_granted") or 0)
    retention = data.get("retention") or {}

    def _ret_line(key: str) -> str:
        row = retention.get(key) or {}
        eligible = int(row.get("eligible") or 0)
        retained = int(row.get("retained") or 0)
        return f"<b>{retained}/{eligible}</b> · {_percent(retained, eligible)}"

    source_lines = []
    for source, count in list((data.get("sources") or {}).items())[:8]:
        source_lines.append(f"• <code>{source}</code> — <b>{count}</b>")

    if not source_lines:
        source_lines = [
            "• Дані джерел з'являться після деплою цієї версії та нових /start."
        ]

    text = (
        f"📈 <b>Growth funnel · {int(data.get('days') or 30)} днів</b>\n\n"
        f"🚪 Відкрили бота: <b>{starts}</b>\n"
        f"⚡ Почали діагностику: <b>{diag_started}</b> · {_percent(diag_started, starts)} від /start\n"
        f"🏁 Завершили діагностику: <b>{diag_completed}</b> · {_percent(diag_completed, starts)} від /start\n"
        f"⚡ Почали Daily: <b>{daily_started}</b> · {_percent(daily_started, starts)} від /start\n"
        f"✅ Завершили Daily: <b>{daily_completed}</b> · {_percent(daily_completed, starts)} від /start\n"
        f"🧠 Почали додаткове тренування: <b>{training_started}</b> · {_percent(training_started, starts)} від /start\n"
        f"✅ Завершили додаткове: <b>{training_completed}</b> · {_percent(training_completed, starts)} від /start\n\n"
        "🔁 <b>Daily retention</b>\n"
        f"D1: {_ret_line('d1')}\n"
        f"D3: {_ret_line('d3')}\n"
        f"D7: {_ret_line('d7')}\n\n"
        f"⭐ Відкрили Premium: <b>{premium_opened}</b> · {_percent(premium_opened, starts)}\n"
        f"💫 Почали оплату Stars: <b>{payment_started}</b> · {_percent(payment_started, starts)}\n"
        f"🎉 Оплатили Stars: <b>{payment_completed}</b> · {_percent(payment_completed, starts)}\n"
        f"💰 Виручка Stars: <b>{int(data.get('stars_revenue') or 0)} ⭐</b> · платників <b>{int(data.get('paying_users') or 0)}</b>\n"
        f"💳 Натиснули оплату менеджеру: <b>{manager_clicked}</b> · {_percent(manager_clicked, starts)}\n\n"
        "🎁 <b>Реферальна петля</b>\n"
        f"🔗 Прийшли за реф. посиланням: <b>{referral_joined}</b>\n"
        f"✅ Зробили Daily у 2 різні дні: <b>{referral_activated}</b> · {_percent(referral_activated, referral_joined)} від реф. переходів\n"
        f"⭐ Видано реферальних нагород: <b>{referral_rewards}</b>\n\n"
        "<b>Джерела першого /start:</b>\n"
        + "\n".join(source_lines)
        + "\n\nДля наступної реклами використовуй окремий start-параметр, наприклад: "
        "<code>?start=ad_channel_0910</code>."
    )
...
@admin_router.callback_query(F.data == "admin_learning_report")
async def admin_learning_report(callback: CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ заборонено.", show_alert=True)
        return
    await callback.answer()
    try:
        data = await LearningDB.admin_learning_report()
        audit = await LearningDB.question_bank_audit()
    except Exception:
        logger.exception("Learning report failed")
        await callback.message.answer("Не вдалося побудувати learning report.")
        return

    events = data.get("events") or {}
    retention = data.get("retention") or {}
    def rr(name):
        item = retention.get(name) or {}
        e = int(item.get("eligible") or 0)
        r = int(item.get("returned") or 0)
        pct = round(r / e * 100, 1) if e else 0
        return f"{name.upper()}: {r}/{e} ({pct}%)"

    latency_lines = []
    for name, item in (data.get("latency") or {}).items():
        latency_lines.append(f"• {name}: p50 {item['p50']}ms · p95 {item['p95']}ms · n={item['n']}")
    if not latency_lines:
        latency_lines = ["• ще немає вимірювань"]

    errors = data.get("errors") or {}
    error_line = ", ".join(f"{k}: {v}" for k, v in errors.items()) or "0"
    sources = data.get("sources") or {}
    source_line = ", ".join(f"{k}: {v}" for k, v in sources.items()) or "—"
    cats = audit.get("categories") or {}
    cat_line = ", ".join(f"{k}: {v}" for k, v in cats.items()) or "—"

    text = (
        "🧭 <b>Learning report · 7 днів</b>\n\n"
        f"Перші заняття: start <b>{events.get('first_lesson_started', 0)}</b> · complete <b>{events.get('first_lesson_completed', 0)}</b>\n"
        f"Daily complete: <b>{events.get('daily_lesson_completed', 0)}</b>\n"
        f"Extra practice: start <b>{events.get('practice_started', 0)}</b> · complete <b>{events.get('practice_completed', 0)}</b>\n"
        f"Course: views <b>{events.get('course_view', 0)}</b> · interest <b>{events.get('course_interest', 0)}</b>\n"
        f"Referrals: visits <b>{events.get('referral_visit', 0)}</b> · activated <b>{events.get('referral_activated', 0)}</b>\n"
        f"Shown without answer: <b>{data.get('dropout_shown_without_answer', 0)}</b>\n\n"
        f"<b>Retention</b>\n{rr('d1')}\n{rr('d3')}\n{rr('d7')}\n\n"
        f"<b>Джерела</b>\n{source_line}\n\n"
        f"<b>Затримки</b>\n" + "\n".join(latency_lines) + "\n\n"
        f"<b>Помилки</b>\n{error_line}\n\n"
        f"<b>Банк</b>\nactive/approved: {audit.get('active_approved', 0)} · {cat_line}"
    )
