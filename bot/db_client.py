...
class DBClient:
    ...
    # ================= USER MANAGEMENT =================
    @classmethod
    def get_or_create_user(cls, user_id: int, username: Optional[str], first_name: Optional[str], referrer_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"user_id": user_id, "streak": 1, "xp": 0, "is_premium": False}

    @classmethod
    def _process_referral(cls, referrer_id: int):
        """Збільшує лічильник рефералів рефереру та дає бонус за кожні 2 друзів."""
        try:
            ...
            # Кожні 2 реферали відкривають Premium або додають бонуси
            if new_count % 2 == 0:
                is_premium = True
    ...
    @classmethod
    def add_xp(cls, user_id: int, amount: int = 10):
    ...
    @classmethod
    def add_question(cls, topic: str, difficulty: int, question_text: str, options: list, correct_option: int, explanation: str = "") -> bool:
        """Додає питання в базу даних (використовується в адмінці)."""
        try:
            payload = {
                "topic": topic,
                "difficulty": difficulty, # 1: Easy, 2: Medium (НМТ пастки), 3: Hard
    ...
    @classmethod
    def get_questions_by_difficulty(cls, difficulty: int, limit: int = 10) -> List[Dict[str, Any]]:
