...
from bot.retention import send_reminder_batch
...
def _level_line(data: Dict[str, Any]) -> str:
    if "xp" not in data:
        return ""
    xp = max(0, int(data.get("xp") or 0))
    level = 1 + xp // 100
    remaining = 100 - xp % 100
    return f"🎮 Ігровий рівень <b>{level}</b> · {xp} XP · до наступного: {remaining} XP\n"
...
async def _send_session_result(message: Message, session: Dict[str, Any]) -> None:
    ...
    motivation = {}
    try:
        session_xp, motivation = await asyncio.gather(
            LearningDB.get_session_xp(str(session.get("id")), int(session.get("user_id"))),
            LearningDB.get_motivation(int(session.get("user_id"))),
        )
    except Exception:
        logger.exception("Could not read session XP")
        session_xp = 0

    ranking_opt_in = motivation.get("leaderboard_opt_in")
    level_line = _level_line(motivation)
    if session_type == "practice":
        ...
        await message.answer(text, reply_markup=get_practice_result_keyboard(had_errors=correct < total, leaderboard_opt_in=ranking_opt_in))
        return
    ...
    text = (
        ...
        f"{streak_line}\n\n"
        f"{next_line}\n\n"
        "Завтра — нова коротка практика й повторення потрібних тем."
    )
    await message.answer(text, reply_markup=get_daily_result_keyboard(had_errors=had_errors, leaderboard_opt_in=ranking_opt_in))
...
async def cmd_start(message: Message, bot: Bot):
    ...
    if not intro_done:
        await message.answer(
            ...
            "Спробуй 3 питання — і одразу побачиш свій перший Neta Memory-профіль.\n\n"
            "💡 Автоматичні підказки — за твоїми налаштуваннями (типово о 19:00 за Києвом). Час і вимкнення — у ⚙️ Налаштування.",
...
async def show_progress(callback: CallbackQuery):
    await callback.answer()
    try:
        data, has_diagnostic, motivation = await asyncio.gather(
            LearningDB.get_progress(callback.from_user.id),
            LearningDB.has_new_diagnostic(callback.from_user.id),
            LearningDB.get_motivation(callback.from_user.id),
        )
    except Exception:
...
async def show_privacy(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "Автоматичні підказки типово ввімкнені о 19:00 за Києвом. За відсутності занять надходять рідше; їх можна вимкнути в один клік.",
...
async def daily_retention_loop(bot: Bot):
    next_send = 0.0
    while True:
        if RETENTION_ENABLED and time.monotonic() >= next_send:
            try:
                delay = await send_reminder_batch(bot, LearningDB)
                next_send = time.monotonic() + delay
            except Exception:
                logger.exception("Reminder loop failed; persisted claims are not resent")
                next_send = time.monotonic() + 60
