...
async def _main_screen(message: Message, user_id: int, edit: bool = False) -> None:
    ...
    if done:
        text = (
            ...
            "✓ Daily виконано. Neta Memory уже зберегла результат і використає його в наступних заняттях.\n\n"
            "Можеш подивитися прогрес або забрати короткі розбори в каналі."
        )
    else:
        text = (
            "<b>Neta · Daily</b>\n\n"
            "<b>4 питання · ~4 хв</b>\n"
            "Нове + повторення того, де ти помилявся. Без довгих уроків і зайвих екранів."
        )
...
async def _send_session_result(message: Message, session: Dict[str, Any]) -> None:
    ...
    text = (
        ...
        f"Точність: <b>{accuracy}%</b>\n"
        f"Сильніше: <b>{strong_line}</b>\n"
        f"На повторення: <b>{focus_line}</b>"
        f"{streak_line}\n\n"
        "<b>Neta Memory оновлено.</b> Наступне заняття врахує сьогоднішні відповіді."
    )
