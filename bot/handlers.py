...
async def cb_show_progress(
    ...
    if simulations:
        ...
        score = latest.get("nmt_score")
        simulation_line = (
            f"🧪 Остання симуляція: <b>{raw}</b> → <b>{score}/200</b>"
            if score is not None
            else f"🧪 Остання симуляція: <b>{raw}</b> → нижче порогу 100 балів"
        )
    else:
        simulation_line = "🧪 Повна симуляція: ще не проходив"
...
async def cb_show_tariffs(
    ...
    text = (
        ...
        "• безлімітні тренування та симуляції\n"
...
async def quiz_full_simulation(callback: CallbackQuery):
    ...
    if not await DBClient.can_start_quiz(user_id):
        ...
        await callback.message.edit_text(
            ...
            "Premium відкриває безлімітні тренування та симуляції.",
            parse_mode="HTML",
            reply_markup=get_tariffs_keyboard(),
        )
        return
    ...
    if len(questions) != FULL_SIMULATION_LENGTH:
        await callback.answer(
            "Для повної симуляції поки недостатньо контенту.",
            show_alert=True,
        )
        return
...
async def _finish_quiz(
    ...
    simulation_note = ""
    if mode == "simulation":
        nmt_score = _nmt_2026_score(correct, total)
        try:
            ...
            logger.exception("Не вдалося зберегти результат симуляції.")

        if nmt_score is None:
            simulation_note = (
                f"\n📌 Тестовий результат: <b>{correct}/{total}</b>. "
                "За таблицею НМТ-2026 це нижче порогу для рейтингового бала 100–200.\n"
            )
        else:
            simulation_note = (
                f"\n🎓 <b>Результат за офіційною таблицею НМТ-2026: {nmt_score}/200</b>\n"
                f"📌 Тестовий бал: <b>{correct}/{total}</b>.\n"
            )
...
async def daily_retention_loop(
    ...
    Wakes once per hour, but sends only during the configured Kyiv hour.
    A user can receive at most one reminder per cooldown window.
    """

    while True:

        try:
            # Worker still wakes hourly, but delivery is restricted to one
            # configured Kyiv-hour window and each user has a 24h cooldown.
