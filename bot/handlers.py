...
async def _main_screen(message: Message, user_id: int, edit: bool = False) -> None:
    ...
    if done:
        text = (
            "<b>⚡ Neta · на сьогодні готово</b>\n\n"
            "✓ Daily виконано\n"
            "🧠 Neta Memory уже оновлена.\n\n"
            "Подивись прогрес або забери короткі розбори НМТ у каналі."
        )
    else:
        text = (
            "<b>⚡ Neta Daily</b>\n\n"
            "<b>4 питання · ~4 хв</b>\n"
            "Нове + те, що Neta Memory повернула тобі на повторення."
        )
...
async def _send_session_result(message: Message, session: Dict[str, Any]) -> None:
    ...
    text = (
        ...
        "🧠 <b>Neta Memory</b> врахує це в наступному занятті."
    )
...
async def show_progress(callback: CallbackQuery):
    ...
    if not attempts:
        ...
        lines = [
            ...
            f"✍️ Відповідей: <b>{attempts}</b>",
        ]
        streak = int(data.get("streak") or 0)
        if streak > 0:
            lines.insert(1, f"🔥 Серія Daily: <b>{streak} дн.</b>")
        if strong:
            lines.append("\n<b>✓ Сильні навички</b>")
...
async def show_friends(callback: CallbackQuery, bot: Bot):
    ...
    medals = ("🥇", "🥈", "🥉")
    ranking = "\n".join(
        f"{medals[i-1] if i <= 3 else f'{i}.'} {html.escape(str(row['alias']))} — <b>{row['days']} дн.</b>"
        for i, row in enumerate(rows, 1)
    ) or "Поки тут тихо — можеш запросити першого друга."
    my_rank = data.get("my_rank")
    my_line = f"\n\nТвоє місце: <b>#{my_rank}</b>" if my_rank else ""
    await callback.message.edit_text(
        "<b>👥 Друзі · цей тиждень</b>\n\n"
        "Рахуємо дні із завершеним заняттям — максимум 1 бал на день. Без нескінченного фарму.\n\n"
        f"{ranking}{my_line}\n\n"
        "Твоє особисте запрошення вже в кнопці нижче.",
...
async def leaderboard_toggle(callback: CallbackQuery, bot: Bot):
    ...
    state = "Ти в рейтингу." if enabled else "Ти вийшов із рейтингу."
    await callback.message.edit_text(
        f"<b>👥 Друзі · цей тиждень</b>\n\n{state} Максимум — 1 бал за навчальний день.\n\n{ranking}\n\nТвоє особисте запрошення — у кнопці нижче.",
...
async def show_course(callback: CallbackQuery):
    ...
    text = (
        "<b>🎓 Neta School</b>\n\n"
        f"{focus_line}"
        "Daily тренує щодня. Курс буде повним маршрутом: <b>короткий урок → практика → повторення → контроль прогресу</b>.\n\n"
        "Курс ще готується і зараз не продається. «Мені цікаво» — лише сигнал, що ти хочеш дізнатися про запуск."
    )
...
async def course_interest(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "<b>🎓 Інтерес збережено</b>\n\nДякую. Це допомагає будувати Neta School за реальним попитом. Поки курс готується, продовжуй Daily — Neta Memory збере точніший профіль навичок.",
